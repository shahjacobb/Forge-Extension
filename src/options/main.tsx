...
const App = () => {
  ...
  const save = async () => {
    ...
    setState(nextState);
    setDraft(nextState.settings);
    setNotice("Saved. Open Miqat on another Chrome profile and sign in to take these with you.");
  };

  if (!state || !draft) {
    return <main className="settings-shell">Loading Miqat settings…</main>;
  }
  ...
  return (
    <main className="settings-shell">
      <section className="settings-card">
        <header>
          <div className="eyebrow">Miqat</div>
          <h1>Settings</h1>
